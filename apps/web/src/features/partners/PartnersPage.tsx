...
export function PartnersPage() {
  ...
  const sortRows = <T extends { updated_at?: string | null; last_editor_name?: string | null }>(rows: T[], label: (row: T) => string) => {
  ...
  const filteredSubcontractors = useMemo(() => sortRows(subcontractors, (row) => row.legal_name), [subcontractors, normalizedQuery, partnersSort]);
  const filteredDrivers = useMemo(() => sortRows(drivers, (row) => `${row.code} ${row.name}`), [drivers, normalizedQuery, partnersSort]);
  const filteredVehicles = useMemo(() => sortRows(vehicles, (row) => `${row.code} ${row.plate_number ?? ''}`), [vehicles, normalizedQuery, partnersSort]);
